...
- **A topic containing a comma is refused**, on subscribe and on publish alike. The comma separates topics in the relay protocol, so one embedded in a name used to connect, report its subscriber counts, and deliver nothing — the worst shape a defect can take, because everything looks like it is working. `subscribe()` now refuses it before opening a socket, throwing with the topic in the message, so the mistake is a stack trace at the call site rather than a connection that closes. It also refuses an empty topic and one padded with whitespace, which fail the same silent way: the relay trims the name, the client filters on the untrimmed one, and nothing is ever delivered. The server refuses the comma too, and logs it, for anything that reaches `/ws` without going through `subscribe()`.
