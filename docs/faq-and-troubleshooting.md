...
- **Login answers `500 {"error":"session write failed"}` while everything else works** — `SESSION_SECRET` is unset, so `SetSession` returns `ErrNoSessionSecret` (`borgo: SESSION_SECRET must be set to use sessions`). The api does *not* refuse to boot over this — an app without sessions is legitimate — so the only signal is the line it logs at startup: `borgo: SESSION_SECRET not set: session and auth routes will fail until it is`. Set a long random string (32+ bytes) and restart; `/healthz` will have been green the whole time.
