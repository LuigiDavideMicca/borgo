...
- **Start with `bunx borgo doctor`** — fourteen checks over the toolchain (bun, a shim shadowing it, go against your `go.mod` floor, node, docker), the machine (both ports, naming the process holding a taken one; free disk) and the project (a stale api process holding the dev binary, generated-types freshness, `node_modules`, app deps, write access to the build directories, playwright browsers if the app depends on playwright). Each failure prints its one-line fix. Checks that do not apply to your app are skipped entirely, and node, docker and the shim note are informational — they print but never change the exit code.
