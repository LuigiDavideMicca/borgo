...
It downgrades itself to `private` when the response *already* carries a `Set-Cookie`, which keeps a shared cache from storing a personalized response — but the check reads the headers at the moment you call it, so the order matters and it fails open: `Cache` then `SetSession` emits `public` on a response that ends up carrying a session cookie. Set cookies first. [Caching](auth-and-sessions.md#caching) covers the rule.
