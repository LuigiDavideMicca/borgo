...
Content-hashed build outputs — the `[name]-[hash].js` chunks — are served `public, max-age=31536000, immutable`, so a returning browser does not even revalidate them. Entry points and `style.css` keep stable names and carry no `Cache-Control` at all, which leaves freshness to the browser's heuristics — with a `Last-Modified` present that is typically a tenth of the file's age, so a returning browser may hold one briefly rather than revalidating every time. A generated service worker is deliberately `no-cache`: a heuristically cached `sw.js` would make every deploy lag behind by however long the browser felt like holding it.
