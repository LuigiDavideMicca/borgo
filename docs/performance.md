...
**The asset index is built once.** In production, one recursive walk of `public/` records, for every file, its size, mtime, content type, cache-control policy, whether a `.br` and a `.gz` sibling exist, and an ETag per variant. Serving a static file is then a `Map` lookup plus one `stat`: no `exists()`, no directory read, no ETag computed per request. The `stat` stays on purpose — one call settles at once whether the file still exists, whether it changed under the index, and whether a precompressed sibling has since been deleted, which an `exists()` cannot. A file written after boot is not in the index and falls back to a live lookup, so nothing breaks; it costs more.
...
**The environment is read once.** Ports, the API base URL, proxy timeouts and retry budgets, the body limit, the CSP template, whether CSRF is enforced — all resolved at boot into closures the handler captures. No `process.env` lookups on any request path a browser reaches, and no chance of a request seeing a half-parsed limit. (One survives, on the internal `Push` endpoint: `BORGO_PUSH_KEY` is read per request, so rotating it takes effect without a restart.)
...
The ETag is derived from the file's size and mtime, and it is **per variant** — the brotli sibling has a different tag from the identity file, because the same URL negotiates to different bytes depending on `Accept-Encoding`. That matters more than it sounds: a shared tag across encodings is how a cache ends up handing brotli bytes to a client that asked for identity. It is a **weak** tag, `W/"…"`, and deliberately so: size and mtime are weak evidence — two files of equal length share the first half, and every tool that copies a file with its date (`cp -p`, `rsync -t`, docker's `COPY`) carries the second across — so the header stops claiming an equality it cannot check. `If-None-Match` compares weakly, so the `304` below still works; what a weak tag gives up is `If-Range`, which is the one place a wrong match would splice new bytes onto an old prefix.

```http
GET /assets/client-a1b2c3d4.js
If-None-Match: W/"i5-mt4ppeo5-br"
...
Content-hashed build outputs are served `public, max-age=31536000, immutable`, so a returning browser does not even revalidate them — and `immutable` is a claim about bytes, so it is made only while the file on disk still has the byte length the build recorded for that name; a file swapped under its hashed name falls back to `no-cache`. A production build names everything it emits after its content — the entry bundle and the stylesheet as well as the split chunks — and your `index.html` goes on referencing `/assets/client.js` and `/assets/style.css`: the server resolves those names against the build's own record when it boots, so nothing in your HTML carries a hash you would have to maintain. A dev build keeps the plain names, since the dev server rebuilds behind a document it already sent. Anything borgo cannot vouch for is served `no-cache`, which costs one bodyless `304` per asset per navigation. A generated service worker is deliberately `no-cache` too: a heuristically cached `sw.js` would make every deploy lag behind by however long the browser felt like holding it.

One class of file is not served at all, whatever the index says: a file whose own name starts with a dot (`.DS_Store`, which carries the listing of its directory; `.gitkeep`; an `.env` somebody dropped into `public/`), and anything under a dot-directory (`public/.git/config`). Both answer `404`. The single exemption is `/.well-known/` as the first path segment, exact case, because RFC 8615 is a standard whose whole point is being fetched — ACME http-01 renews your certificate through it. A nested `.well-known` is not the standard's and is refused like any other dot-directory. `borgo export` applies the same rule before copying `public/`, and the `precache.json` a service worker installs from lists only what the build emitted, never a dotfile.

Range requests get one subtlety right that matters for correctness rather than speed. `If-Range` makes a range conditional on the client still holding the representation it started downloading, and RFC 9110 requires a *strong* validator on both sides before a range may be honoured. Because a borgo asset URL negotiates to different bytes per encoding, the mismatch is not exotic — a resume arriving without the original `Accept-Encoding` is asking for a slice of the brotli file to be filled from the identity one — and because borgo's validators are weak, every `If-Range` on an asset gets the full file instead, still streamed off disk, never buffered through memory. A bare `Range` with no `If-Range` is untouched, so media seeking is unaffected; the cost is confined to interrupted downloads, and it is the direction the RFC requires.
...
**Pre-encoded and streaming responses pass through.** The Go middleware detects `text/event-stream` and an existing `Content-Encoding` at `WriteHeader` time and switches to passthrough, so an event stream is never buffered waiting to reach a compression threshold, and `Flush` on a still-buffering response commits it as identity immediately rather than holding it. Both readings are of the *whole* header, under every spelling of the key: a handler that added `text/plain` before `text/event-stream`, or assigned `h["content-type"]` through the map, used to walk past the gate and get gzip put over its stream — the browser folds the key and reads every line, so borgo does too. The reverse mistake is covered as well: an empty `Content-Encoding` value names no coding, so `["", "br"]` is read as br and left alone rather than compressed twice and mislabelled. `1xx` responses pass straight through, trailers survive whatever the body length, and a `204`/`304` is never given an encoding over zero bytes.

**A `Content-Length` the handler declared is kept honest, not trusted.** Under gzip it is deleted — it describes the uncompressed body. Under identity, a second contradictory line is dropped (the first is the one `net/http` sized the body by; two of them make a client drop the connection with no status), and a length the body has already outgrown at commit is deleted before the header leaves, instead of reaching a client that then reads a truncated body as `unexpected EOF`. Either way the mismatch is logged, once per response — `Content-Length N but wrote M bytes` — and `M` is what the wire accepted, not what the handler offered, so a client that hung up mid-download is not reported as an application bug.
...
**The event stream is not buffered anywhere, and one line of it is not decoration.** `borgo.SSE` writes a `:ok` comment — an SSE frame every client ignores — before it returns the stream to your handler, and without it a stream that stays quiet until its first real event leaves the browser hanging on `fetch()` for exactly that long. Flushing the headers is not enough: `Bun.serve`, which is borgo's *own* front server, does not send a response's headers downstream until the first body byte arrives. The `X-Accel-Buffering: no` header asks nginx and friends for the same favour, but they are the intermediaries you might not have; the one you always have is borgo's. Beyond that, the proxy forwards without a body deadline once headers have arrived, and the front server's socket read deadline — 30 s, because the same number is what stops a slow client dribbling a request body — is **kept warm** for the one request carrying the stream rather than lifted for the connection. The trigger is not the response's `Content-Type` — that was tried and was wrong in kind, because it truncates every long-lived response outside the allowlist and arrives only after the handler has resolved. It is the request: the moment the client provably has nothing left to send, which for an event stream is at the top of `fetch()`, since a GET carries no body at all. From there a shared two-second sweep re-arms a short deadline for as long as the stream is in flight and stops when it ends, so the stream is not cut, every other path keeps its deadline, and nothing is left disarmed behind it. Two corollaries are worth knowing: a stream reached by a POST whose body borgo never finished reading keeps the deadline for its whole life, and there is one declared limitation under heavy load — see [realtime](realtime.md#streams-and-server-timeouts). [Realtime](realtime.md#server-sent-events) has the handler side.
