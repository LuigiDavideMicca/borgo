...
- **Component, page and hook edits apply in place** through the [react-refresh](https://www.npmjs.com/package/react-refresh) runtime, instrumented by bun's own native fast-refresh transform (dev builds only — Babel left in 0.22, see [the transform, since 0.22](#the-transform-since-022)). The current route's new chunk is imported, loader props are refetched, and component state survives a body edit. Change a component's *hooks* — add, remove, reorder, or alter a custom hook's signature — and just that component remounts, Next-style; the rest of the page keeps its state. Editing a custom hook's body hot-applies with dependent state intact.
...
## The transform, since 0.22

Through 0.21, dev builds instrumented components for fast refresh with Babel and its react-refresh plugin — the same machinery Vite and Next use, and the single heaviest dependency in the tree. 0.22 removes it. Bun's bundler carries its own native fast-refresh pass, and borgo's dev build now turns it on directly; the react-refresh *runtime* in the browser is unchanged, only the compile-time instrumentation moved.

The honest print: the option is undocumented in Bun, so borgo pins the behaviour rather than trusting it. A suite of pipeline tests asserts what the pass must emit — components registered with the module baked into the id, hook signatures that change when the hooks change (which is what remounts a component), custom hooks in plain `.ts` carrying signatures, and no refresh machinery at all in a module with no components. If a Bun upgrade renames or drops the option, those tests go red by name instead of hot reload dying mute.

Two emit defects of Bun 1.4.0's pass are patched after the bundle, and both patches are quarantined behind invariants that cannot break working code. The first defect emits `const X = _s(...)` followed by `var X = X;` — an illegal redeclaration, so a file carrying that pair could never have parsed, and deleting the line cannot change the behaviour of anything that ran. The second renames the refresh globals (`$RefreshSig$2`) when chunks collide; the reference is pointed back at the real global only when nothing in the chunk declares the renamed name, meaning it was about to throw. Both passes read the code through a mask that blanks string, template and comment content, because the adversarial run found them deleting lines out of user template literals and rewriting `$RefreshSig$N` inside strings — string content is data, never edited. And both are built to be deleted: the day Bun stops emitting either shape, the pinning tests pass with zero lines removed, and that is the signal the shim goes.

The move raised borgo's floors to **Bun 1.4** and **Go 1.27**, together, so there is one requirements change instead of two releases apart. Bun 1.4 is where the native pass emits what the tests pin — and where `bun build` on Windows stopped mangling `file://` paths — and Go 1.27 is the toolchain the release is actually verified against: a floor the CI matrix has never run is a claim, not a floor. If your machine runs an older Bun *somewhere* — an npm-installed shim, a stale `~/.bun` shadowed by a newer install on `PATH` — `borgo doctor` names it: two Buns of different versions on one machine is exactly the class of problem that produced a dev-only render crash during 0.22's own development.
