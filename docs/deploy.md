...
**The body cap is the app's, not a number.** `borgo.Bind` reads at most 1 MiB and answers `413` above it (`borgo.BindError` picks the status); a route that legitimately takes more declares it with `borgo.BindMax`, and then the proxy line moves with it. A proxy that lets more through does not make the app accept more — it only moves the refusal one hop later, into a Go handler, where nothing in the proxy's logs explains it. The front server's own `BORGO_MAX_BODY` (32 MB) is a different ceiling: the most it will *read* on the way through, not what a handler will decode. It is counted byte by byte as the body arrives, so how the body is framed does not change where the refusal happens.
