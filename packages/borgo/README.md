...
**The self-hosted React framework.** File-based React pages server-rendered by Bun; API routes written in Go, with the TypeScript types for every route **generated from the Go source** — rename a Go field and `tsc` fails on the page that read it. Deploy is one static Go binary and one Bun server on any box you control: no Vercel, no platform bill, [by design](https://github.com/LuigiDavideMicca/borgo/blob/main/docs/why.md).

```bash
bunx create-borgo@latest my-app
```

A page is a file; the route is its name; the server data is a `loader` beside the component, typed against the Go handler it calls:
...
export async function loader({ params, api }: LoaderContext) {
  return await api("GET /api/tasks/{id}", { params: { id: params.id } });
}
```

And every page picks how it ships, with one export: nothing for per-request SSR (streamed through Suspense), `revalidate`/`tags` for cached pages Go invalidates on demand, `prerender` for static export, `hydrate = false` for zero JavaScript, `"visible"` to defer it, islands in between. The [repository README](https://github.com/LuigiDavideMicca/borgo#every-page-picks-how-it-ships) shows the whole menu; the [docs](https://github.com/LuigiDavideMicca/borgo/blob/main/docs/README.md) go deep on each.

This package is the Bun/TypeScript core: SSR front server, file-based router, build pipeline, dev orchestrator and the `borgo` CLI. It ships its TypeScript source directly — Bun runs it natively, and what you read on npm is what runs. Requires Bun >= 1.4 and Go >= 1.27 (`bunx borgo doctor` checks both, and most of your machine besides).
...
Most of 0.22 arrives without touching existing code — cached pages (`revalidate`/`tags` + `borgo.RevalidateTag` from Go), the typed environment schema (`env.ts`), `@/` imports — but a few behaviours changed on purpose:

- **The floors move together: Bun >= 1.4 and Go >= 1.27.** One requirements change, both proven by CI rather than assumed. On an older machine `bunx borgo doctor` names the version and the one-line upgrade — and it now also catches a *second*, older Bun hiding in `~/.bun` that npm bin shims execute behind your `PATH`'s back.
- **Babel left the tree.** Dev-build fast refresh is instrumented by Bun's own native transform; the react-refresh runtime in the browser is unchanged. You should notice nothing except the install getting lighter — the [transform notes](https://github.com/LuigiDavideMicca/borgo/blob/main/docs/dev-experience.md#the-transform-since-022) say what is pinned by tests.
- **A route now outranks an extension-less file.** An accidental `public/about` file no longer shadows `pages/about.tsx` for `/about`. Paths *with* an extension behave exactly as before.
- **`borgo start` defaults `NODE_ENV=production`** onto the launch environment when nothing set it, so production serves production React (measured +36% on SSR). Set `NODE_ENV` to a real value yourself and borgo leaves it alone; an *empty* `NODE_ENV` counts as unset, deliberately.
- **`borgo export` skips `revalidate` pages by name** — a page that asked for a server is not silently baked static.
- Scaffolded apps now declare TypeScript 7 in devDependencies; apps generated by older create-borgo keep 5.9 and keep working.

## Upgrading from 0.20

Every line below is a behaviour that changed on purpose between 0.20.1 and 0.21. The full list is in the [changelog](https://github.com/LuigiDavideMicca/borgo/blob/main/packages/borgo/CHANGELOG.md); these are the ones an existing app meets first.
