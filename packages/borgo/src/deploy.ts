...
export function nginxConf({ name, port }: DeployContext): string {
  return [
    ...
    "# the client sent an Upgrade header and the empty string otherwise, which",
    "# makes nginx omit the Connection header entirely - no hop-by-hop on",
    "# ordinary requests, and the upstream connection free to keep alive. it",
    "# sits at http level, which is where sites-enabled is included from.",
