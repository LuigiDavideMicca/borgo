...
export function systemdUnit({ name, port, apiPort, secret }: DeployContext): string {
  ...
  // 32-byte floor, so the api refused to boot at all - CheckEnv returns the
  // refusal and Serve turns it into a fatal, before the listener binds. And
  // because a real environment variable beats `.env` in bun, the unit borgo
  // wrote also overrode the random key create-borgo had generated. An app that
  // worked locally, stopped from starting by its own deploy config. So: when
  // the app already has a key, the unit deliberately sets nothing and says why;
  // when it does not, the key written here is a real one.
  const session = secret
    ...
    : [
        ...
        "# SESSION_SECRET under 32 bytes at startup, so a placeholder here would",
        "# be an api that does not boot at all - not one that starts and fails",
        "# later. shipping a .env instead? delete this line: bun reads .env from",
        "# the working directory, and a real environment variable would override it.",
